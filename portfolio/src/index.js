import { i18Obj } from '../src/utils/translate';
import { lightThemeTags } from '../src/utils/light-theme';
...
portfolioWinterBtn.addEventListener('click', () => {
  portfolioImages.forEach(
    (img, index) => (img.src = `../src/assets/img/winter/${index + 1}.jpg`)
  );
});

portfolioSummerBtn.addEventListener('click', () => {
  portfolioImages.forEach(
    (img, index) => (img.src = `../src/assets/img/summer/${index + 1}.jpg`)
  );
});

portfolioSpringBtn.addEventListener('click', () => {
  portfolioImages.forEach(
    (img, index) => (img.src = `../src/assets/img/spring/${index + 1}.jpg`)
  );
});

portfolioAutumnBtn.addEventListener('click', () => {
  portfolioImages.forEach(
    (img, index) => (img.src = `../src/assets/img/autumn/${index + 1}.jpg`)
  );
});
